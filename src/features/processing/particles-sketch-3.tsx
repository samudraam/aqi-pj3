...
// COMMENTED OUT: MediaPipe interfaces
...
declare global {
  interface Window {
    // COMMENTED OUT: MediaPipe globals
...
// COMMENTED OUT: Mouth tracking interface
...
interface ParticlesSketch3Props {
  showControls?: boolean;
}

const ParticlesSketch3 = ({ showControls = true }: ParticlesSketch3Props) => {
  const {
    airQualityDetails,
    fetchCityAirQuality,
    cityQuery,
  ...
  // COMMENTED OUT: Video ref for MediaPipe camera
  // const videoRef = useRef<HTMLVideoElement | null>(null);

  // COMMENTED OUT: MediaPipe and camera state
  ...
  // COMMENTED OUT: MediaPipe script loading
  // useEffect(() => {
  ...
  /** Main p5 sketch */
  useEffect(() => {
    // COMMENTED OUT: Removed mediaPipeLoaded check
    if (!sketchRef.current || !airQualityDetails) return;

    let sketchInstance: p5 | null = null;
    // COMMENTED OUT: Camera stop function
    ...
    const sketch = (s: p5) => {
      ...
      // COMMENTED OUT: Mouth tracking state
      ...
      // --- Helpers ---
      // COMMENTED OUT: clamp01 helper (was used for mouth tracking)
      ...
      // COMMENTED OUT: dist2D helper (was used for face tracking)
      // const dist2D = (
      //   a: { x: number; y: number },
      //   b: { x: number; y: number }
      // ) => {
      ...
      /** Vortex overlay */
      // COMMENTED OUT: Vortex overlay (depends on mouth tracking)
      ...
      // COMMENTED OUT: Face tracking setup
      ...
      s.setup = () => {
        ...
        // COMMENTED OUT: Camera capture setup
        ...
        // COMMENTED OUT: Camera ready flag
        // camReady = true;
      };
      ...
      s.draw = () => {
        ...
        // Solid background instead of camera
        s.background(255);

        // COMMENTED OUT: Mouth tracking coordinate conversion
        // if (mouth._vx !== null && mouth._vy !== null) {
        //   mouth.x = mouth._vx * s.width;
        //   mouth.y = mouth._vy * s.height;
        // }

        // COMMENTED OUT: Mouth open scale
        ...
        for (let i = shapes.length - 1; i >= 0; i--) {
          ...
          // COMMENTED OUT: Mouth interaction with particles
        ...
        // COMMENTED OUT: Vortex overlay drawing
        // drawVortexOverlay();
      };

      s.windowResized = () => {
        s.resizeCanvas(s.windowWidth, s.windowHeight);
        // COMMENTED OUT: Camera resize
        // if (cam) {
        //   (cam as p5.MediaElement).size(s.windowWidth, s.windowHeight);
        // }
        computeClusterCenters();
    ...
    return () => {
      ...
      // COMMENTED OUT: Camera stop cleanup
      // if (cameraStopFn) {
      //   cameraStopFn();
      // }
    };
    // COMMENTED OUT: Removed mediaPipeLoaded from dependencies
  }, [airQualityDetails]);

  return (
    <>
      {/* COMMENTED OUT: Video element for MediaPipe camera */}
      {/* <video
        ref={videoRef}
        style={{ display: "none" }}
        playsInline
      ...
      {/* COMMENTED OUT: Camera error display */}
      {/* {cameraError && (
        <div
          style={{
            position: "fixed",
      ...
      {showControls && !airQualityDetails && (
        <div
          style={{
            position: "fixed",
            inset: "50% auto auto 50%",
...
export default ParticlesSketch3;
